...
export function TripDecisionMap({ nodes = [], comparisonNodes = [], activeNodeId = null, onFocusNode, mobility = null, comparisonMobility = null, tripId = null, staticMapAvailable = false, label = "旅行地点地图", locale = "zh-CN" }) {
  ...
  return <section className="trip-decision-map" aria-label={label}>
    ...
    <footer><span>{useInteractive ? (english ? "Interactive comparison map" : "互动位置对比地图") : useStatic ? (english ? "Amap map snapshot" : "高德地图快照") : (english ? "Map pending" : "地图待补")}</span><small>{useInteractive ? comparisonPathCount ? (english ? "Blue lines use the recommended mode. Gray dashed lines use another returned mode only where the recommended geometry is unavailable." : "蓝色实线为当前方式；灰色虚线只在当前折线缺失时展示同一路段的其他可用方式，具体方式以时间轴为准。") : (english ? "This basemap is for spatial comparison. Final navigation continues in Amap." : "这张底图用于位置比较；正式导航仍使用高德。") : (english ? "Maps and routes show only data returned by named sources." : "地图与路线均只展示具名来源返回的资料。")}</small></footer>
  </section>;
}
