...
- **2026-08-29 真实用户审计的 14 项问题已修复并同场景复验（[审计与 Fix Checklist](./research/2026-08-29-real-user-product-audit-and-fix-checklist.md) §12）：** A0 现已具备 arrivalAt、Day/Date/Role、住宿回环和确认前 feasibility gate；航班/高铁、有效路线基准、模式比较、住宿目标区实际动线、长尾餐饮排序、等待后台继续、CTA/44px/用户文案也已收敛。真机、生产 OAuth、实时设施和独立社交证据仍按上线门单列。
...
- **真实 AI 行程优化闭环已补齐（2026-08-30）：** `plan-trip` 现在由 Parent 直接生成 canonical `ItineraryPlan`，唯一的 `plan_itinerary_trial` 使用高德路线与确定性 Checker 核验，并最多允许一次修正；旧固定站序降级为明确的 quick fallback。桌面按钮不再预填对话，393px 端逐段模式会在服务端重新检查。上海真实路径完成 Trial、1145m>600m 阻断、切回打车和 revision 1→2 确认；详见[迭代文档 §13](./current/11-intelligent-planning-iteration.md)、[动态并行架构 §15](./current/12-agent-runtime-and-parallelism-architecture.md)与[真实审计 §13](./research/2026-08-29-real-user-product-audit-and-fix-checklist.md)。
