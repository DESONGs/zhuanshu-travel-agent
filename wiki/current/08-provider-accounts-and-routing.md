...
| 微信小程序 | 在微信公众平台创建小程序、登记服务器域名、配置 OAuth/登录回调及隐私合规材料 | 实现并验证 `platform-exchange`，替换开发登录 | 微信公众平台开发者后台（账号主体操作） |
| 支付宝小程序 | 在支付宝开放平台创建应用、配置签名/授权回调和服务器域名 | 实现并验证 `platform-exchange`，替换开发登录 | 支付宝开放平台开发者后台（账号主体操作） |
