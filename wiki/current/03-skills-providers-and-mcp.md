...
22 个 Skill 以“研究计划 → 受限获取 → Claim/Entity 归一 → 天气/位置联合评估 → Patch → QA”为稳定编排；它只在一次 `research_trip_options` 内有界调用，所有结果形成一份可选择的跨域提案，不产生四个孤立 Workflow。天气 Provider 是 Tool/Adapter，Runtime Environment Gate 强制保证天气新鲜度与上下文存在；`assess-trip-weather` 只评估 Runtime 已提供且覆盖旅行日期的天气，并把影响映射回四条任务链。高德、天气来源与飞猪 FlyAI 由同一个组合 Provider 汇合：高德补全地点和地图，天气先约束最终候选与取舍，FlyAI 补充酒店、景点、火车与航班商业库存及飞猪跳转；来源归一、去重后才生成同一份提案。FlyAI 不承担美食口碑。

首次旅行研究由 Runtime 强制覆盖吃、住、行、玩四条任务链，即使模型只请求了其中一域；已有提案或已选方案后的局部调整才尊重受影响域子集。Provider 暂时无结果时保留缺失域，不用模型知识补齐。

## Provider 分层

模型路由独立于旅行事实来源：DeepSeek 负责 Parent Agent 的推理和受限工具调用；Kimi 仅在用户授权图片确实需要视觉理解时处理，输出仍是待审查 Claim。具体账号、状态与验证见 [模型路由、账号接入与数据能力](./08-provider-accounts-and-routing.md)。
...
| 铁路 | 飞猪 FlyAI；途牛官方 MCP；购买时跳转 12306 或授权渠道 | 两个个人 Key 均已接入并通过真实只读 smoke；无公开官方 12306 开发 API，且不自动购票。 |
| 航班/酒店/景点库存 | 飞猪 FlyAI；途牛官方 MCP 为第二来源 | FlyAI 已接搜索、图片、价格提示和详情深链；价格与库存仍需进入供应方页面再次核验，不自动购买。 |
